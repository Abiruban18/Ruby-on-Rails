...
# --- Inheritance: DigitalBook Subclass ---

class DigitalBook < Book
  attr_accessor :url

  def initialize(title, author, genre, year, url)
    # super calls the initialize method of the parent Book class
    super(title, author, genre, year)
    @url = url
  end

  # Overriding display to append subclass information cleanly
  def display
    super
    puts "URL    : #{@url}"
    puts "-" * 30
...
class Library
  ...
  def list(limit: nil)
    ...
    collection.each do |book|
      book.display
      # If it's a plain book, add the trailing visual separator line 
      # (DigitalBook handles its own trailing line in its overridden display)
      puts "-" * 30 unless book.is_a?(DigitalBook)
    end
  end
...
def show_menu
  ...
  puts "9. Add a digital book" # Assigned to Option 9
  puts "10. Detailed Library Stats"
  puts "11. Export book to clipboard format"
  puts "12. Library summary"
  print "\nSelect an option: "
end
...
def validate_year!(value)
  ...
  # Matches exact digits sequence to assert it is numeric
...
# Handling Runtime Custom and System Exceptions Defensively
rescue BookNotFoundError => e
  puts "\n[Application Error] #{e.message}"
  retry # Restarts the begin block loop execution safely
rescue InvalidInputError => e
  puts "\n[Validation Failure] #{e.message}"
  retry
rescue Interrupt
  # Catches system level Ctrl+C events gracefully
  puts "\nProcess interrupted."
ensure
